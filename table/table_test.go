...
// TestMoveUpFromBottomKeepsViewport checks that stepping up from the last row
// only moves the cursor: the visible window must not scroll while the cursor
// is still inside it.
func TestMoveUpFromBottomKeepsViewport(t *testing.T) {
	rows := make([]Row, 30)
	for i := range rows {
